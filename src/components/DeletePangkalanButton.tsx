...
export default function DeletePangkalanButton({ pangkalanId, namaPangkalan, redirectTo = '/pangkalan' }: Props) {
  ...
  const handleDelete = async () => {
    ...
    const { error: deleteError } = await supabase.from('pangkalan').delete().eq('id', pangkalanId)
    if (deleteError) {
      setError('Gagal menghapus pangkalan. Coba lagi.')
      setLoading(false)
      return
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6 animate-scale-in">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-base font-bold text-slate-800 mb-2">Hapus Pangkalan?</h3>
              <p className="text-slate-500 text-sm mb-1 leading-relaxed">
                Pangkalan <strong className="text-slate-700">"{namaPangkalan}"</strong> beserta semua data distribusinya akan dihapus permanen.
              </p>
              <p className="text-xs text-red-400 mb-5">Tindakan ini tidak dapat dibatalkan.</p>

              {error && <p className="text-xs text-red-500 mb-3">{error}</p>}

              <button id="btn-konfirmasi-delete-pangkalan" onClick={handleDelete} disabled={loading}
